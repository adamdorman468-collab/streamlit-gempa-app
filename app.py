...
with col2:
    # --- PERBAIKAN: Panggil fungsi jam JavaScript di sini ---
...
if df_gempa.empty:
    ...
    # ... (Sisa kode untuk filter, peta, tabel, dll. TIDAK ADA YANG DIUBAH) ...
    ...
    with filter_col1:
        ...
        if min_mag == max_mag:
            mag_val = st.number_input("Magnitudo (satu nilai):", min_value=min_mag, max_value=max_mag, value=min_mag, format="%.1f")
            ...
            mag_filter_values = st.slider("Saring berdasarkan Magnitudo (rentang):", min_value=min_mag, max_value=max_mag, value=(low, high), step=step)
        if only_ge_5:
